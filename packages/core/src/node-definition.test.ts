...
import { resolveInputSocket, resolveParamDefaults, type NodeDefinition } from './node-definition';
...
describe('resolveInputSocket', () => {
  it('finds an input socket by name, else undefined', () => {
    expect(resolveInputSocket(vectorMath, 'a')).toEqual({ name: 'a', type: 'Vector' });
    expect(resolveInputSocket(vectorMath, 'nope')).toBeUndefined();
  });
});
