...
- [x] **Scope:** topological walk → straight-line TS, wiring outputs→inputs via locals; import
...
- [x] **Scope:** root network → `export default function <name>(...)`; name sanitized from
...
- [x] **Scope:** same generator emitting an ES module without type annotations.
- **Acceptance:** generated `.js` runs in Node.
- **Branch:** `phase-6/step-6.3-js-target`

## Step 6.4 — Main-thread wrapper

- [x] **Scope:** synchronous wrapper exposing the default-export function directly.
- **Acceptance:** import + call works.
- **Branch:** `phase-6/step-6.4-main-thread-wrapper`

## Step 6.5 — Worker wrapper

- [x] **Scope:** Web Worker wrapper (Comlink-style RPC) exposing an async same-name/args fn.
...
- [x] **Scope:** test harness comparing interpreter output to compiled-module output for example
      networks.
